...
class Git_igniter {
	...
	public function __construct(){
		...
		$this->ci->load->config('git_igniter', TRUE);
		
		$this->client_id = $this->ci->config->item( 'gh_client_id', 'git_igniter' );
		$this->secret = $this->ci->config->item( 'gh_secret', 'git_igniter' );
		$this->api_url = $this->ci->config->item( 'gh_api_url', 'git_igniter' );
		$this->auth_url = $this->ci->config->item( 'gh_auth_url', 'git_igniter' );
		$this->token_url = $this->ci->config->item( 'gh_token_url', 'git_igniter' );
		$this->org = $this->ci->config->item( 'gh_org', 'git_igniter' );
		$this->redir_url = $this->ci->config->item( 'gh_redir_url', 'git_igniter' );
		$this->scope = $this->ci->config->item( 'gh_scope', 'git_igniter' );
		
		$this->ci->load->model('git_igniter_model');
	...
	*********************************************************************************************************************/
	...
	public function store_token( $token, $user_id, $session = TRUE, $db = TRUE ){
		...
		if( $db === TRUE )
		{
			...
			$this->ci->git_igniter_model->store_token( $token, $user_id );
	...
	*********************************************************************************************************************/
	...
	public function retrieve_token( $user_id ){
		...
		$token =  $this->ci->git_igniter_model->retrieve_token( $user_id );
